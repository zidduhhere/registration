...
const Register = () => {
  ...
  const [eventSelected, setEventSelected] = useState<EventGuidelines | null>(
    null,
  );
    ...
    if (saved) {
      try {
        ...
        if (parsedData.eventId) {
          const event = eventGuidelines.find(
            (e) => e.eventId === parsedData.eventId,
          );
          if (event) {
  ...
  const onSubmit = async (data: any) => {
    ...
    let fileInput = data.fileUpload?.[0];

    // Handle Offline Payment
    if (data.offlinePayment) {
      try {
        const response = await fetch("/offline.jpg");
        const blob = await response.blob();
        fileInput = new File([blob], "offline.jpg", { type: "image/jpeg" });
      } catch (error) {
        console.error("Error loading offline payment placeholder:", error);
        alert("Error processing offline payment option. Please try again.");
        return;
      }
    } else {
      if (!fileInput) {
      ...
      if (!allowedFormats.includes(fileInput.type)) {
        alert(
          "Invalid file format! Please upload an image file (JPG, JPEG, PNG, or WEBP only).",
        );
        return;
      }
  ...
  const validateStep = async (step: number): Promise<boolean> => {
    switch (step) {
      ...
      case 4:
        ...
        return true;
      case 5:
        // Validate file upload if not offline
  ...
  return (
    <div className="flex flex-col justify-center items-center gap-12 w-screen">
        ...
        <div className="fixed inset-0 bg-white bg-opacity-80 backdrop-blur-sm flex items-center justify-center z-50">
          <div className="bg-white p-8 rounded-lg clash text-center shadow-2xl border-2 border-gray-200">
            ...
            <p className="text-sm text-gray-600 mt-2">
              Please wait, uploading your data
            </p>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white p-8 rounded-lg clash max-w-md">
            ...
            <p className="mb-4 serif">
              Your registration could not be completed. Would you like to retry
              with your saved details?
            </p>
            <p className="text-sm text-gray-600 mb-6 serif">
              All your entered information has been saved. You can retry the
              submission without re-entering your details.
            </p>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white p-8 rounded-lg clash max-w-md text-center">
            ...
            <p className="text-sm text-gray-600 mb-6 serif">
              ...
              <strong>{savedFormData?.email}</strong>
            </p>
            <button
              onClick={() => {
      ...
      {savedFormData && (
        <div className="fixed top-20 right-4 bg-blue-100 border-2 border-blue-400 text-blue-700 px-6 py-4 rounded-lg shadow-lg z-40 max-w-sm">
          <div className="flex items-start gap-2">
            ...
            <div>
              <strong className="font-bold clash">
                Previous Data Restored
              </strong>
              <p className="text-sm mt-1 serif">
                Your previously entered information has been loaded.
              </p>
              <button
                onClick={() => {
                  sessionStorage.removeItem("registrationFormData");
      ...
      <div
        onClick={() => navigate(-1)}
        className="w-40 text-center clash font-semibold bg-black absolute left-7 top-10 px-4 py-2  text-white cursor-pointer hover:bg-gray-800"
      >
        Back
      </div>
      ...
      <div className="body w-full mx-auto">
        <Stepper
          ...
          backButtonProps={{
            className:
              "bg-black text-white px-4 py-2 rounded-md clash hover:bg-gray-800",
          }}
          nextButtonProps={{
            className:
        ...
        >
          ...
          <Step>
            <ScreenshotUpload
              register={register}
              errors={errors}
              onFileValidityChange={setIsFileValid}
              watch={watch}
              setValue={setValue}
            />
          </Step>
